...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        <td>'. $row["emp_type"].'</td>
        <td><a class="btn btn-dark" href="emp_form_edit.php?id='. $row["emp_type_id"].'">Edit</a>
        <a class="btn btn-danger" 
        onclick="return confirm(\'Are you sure?\')" href=" delete_employee.php?id='. $row["emp_type_id"].' " 
        "
        > Delete </a>
        </td>
        </tr>
